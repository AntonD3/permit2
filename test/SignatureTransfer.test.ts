...
describe('SignatureTransferTest', function () {
    ...
    it('should correct computing typehashes', async function () {
      expect(
        ethers.utils.keccak256(
          ethers.utils.solidityPack(['string', 'string'], [_PERMIT_TRANSFER_TYPEHASH_STUB, WITNESS_TYPE_STRING])
        )
  ...
  describe('Test Permit TransferFrom To Spender', function () {
    it('should correct work transferFrom to spender ', async function () {
      ...
      expect(await token0.connect(owner).balanceOf(owner.address)).to.be.equal(startBalanceFrom.sub(defaultAmount))
      expect(await token0.connect(owner).balanceOf(ethers.constants.AddressZero)).to.be.equal(
  ...
  describe('Test Permit Batch TransferFrom', function () {
    it('should correct work permitBatch transferFrom ', async function () {
      ...
      expect(await token1.connect(owner).balanceOf(ethers.constants.AddressZero)).to.be.equal(
  ...
  describe('Test PermitBatchTransfer Invalid Amounts Length Mismatch', function () {
    it('should revert permitBatch invalid amounts length mismatch ', async function () {
      ...
      ).to.be.revertedWithCustomError(permit2, 'LengthMismatch')
    })
  })
    ...
    it('should work permitBatchTransferFrom with typed witness ', async function () {
      ...
      await (
        await permit2
          ...
          ](permit, toAmountPairs, owner.address, witness, WITNESS_TYPE_STRING, sig)
      ).wait()
      ...
      await expect(
        permit2
          ...
          [
            ...
            owner.address,
            ethers.utils.keccak256(
              ethers.utils.solidityPack(['bytes32'], [ethers.utils.formatBytes32String('bad witness')])
            ),
            WITNESS_TYPE_STRING,
            sig
          )
    ...
    it('should work permitBatchTransferFrom with typed witness ', async function () {
      ...
      await (
        await permit2
          ...
          [
            'permitWitnessTransferFrom(((address,uint256),uint256,uint256),(address,uint256),address,bytes32,string,bytes)'
          ](permit, transferDetails, owner.address, witness, WITNESS_TYPE_STRING, sig)
      ).wait()
    ...
    it('should revert permitBatchTransferFromTyped Witness with invalid type ', async function () {
      ...
      await expect(
        permit2
          ...
          [
            'permitWitnessTransferFrom(((address,uint256),uint256,uint256),(address,uint256),address,bytes32,string,bytes)'
          ](permit, transferDetails, owner.address, witness, ethers.utils.formatBytes32String('fake typedef'), sig)
      ).to.be.revertedWithCustomError(permit2, 'InvalidSigner')
    })
  })
