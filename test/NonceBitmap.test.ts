...
const RICH_WALLET_PRIVATE_KEYS = JSON.parse(
  fs.readFileSync("test/shared/rich-wallets.json", "utf8")
);
...
describe("NonceBitmap", function () {
  ...
  describe("Test Invalidate Full Word", function () {
    it("should fail 4", async function () {
      ...
      await expect(
        permit2.useUnorderedNonce(spender.address, ethers.BigNumber.from(0))
      ).to.be.revertedWithCustomError(permit2, "InvalidNonce");
      await expect(
        permit2.useUnorderedNonce(spender.address, ethers.BigNumber.from(1))
      ).to.be.revertedWithCustomError(permit2, "InvalidNonce");
      await expect(
  ...
  describe("Test Using Nonce Twice Fails", function () {
    it("should fail 6", async function () {
      ...
      await expect(
        permit2.useUnorderedNonce(spender.address, nonce)
      ).to.be.revertedWithCustomError(permit2, "InvalidNonce");
    });
  });
  ...
  describe("Test Use TwoRandom Nonces", function () {
    it("should fail 7", async function () {
      ...
      if (first == second) {
        await expect(
          permit2.useUnorderedNonce(spender.address, second)
        ).to.be.revertedWithCustomError(permit2, "InvalidNonce");
      } else {
